...
public class AI
{
    ...
    public ushort GetBestMove(int maxDepth)
    {
        ...
        bool isWhite = Board.Instance.isWhiteTurn;
        int previousScore = 0;
        
        int actualMaxDepth = Math.Min(maxDepth, 63);
            ...
            while (true) // Aspiration Loop
            {
                ...
                for (int i = 0; i < currentMoveCount; i++)
                {
                    ...
                    if (Chess.Instance.isSquareSafe(currentKing))
                    {
                        ...
                        Board.Instance.isWhiteTurn = !isWhite;
    ...
    private int Search(int depth, int ply, int alpha, int beta, bool allowNull)
    {
        ...
        if (ply >= 63) 
        {
            ...
            return Board.Instance.isWhiteTurn ? eval : -eval;
        }
    ...
    private int QuiescenceSearch(int ply, int alpha, int beta)
    {
        ...
        standPat = Board.Instance.isWhiteTurn ? standPat : -standPat;
    ...
    public int EvaluateBoard()
    {
        ...
        if (Math.Abs(material) >= 300 && phase <= 6)
        {
    ...
    private int EvaluateMopUp(bool isWhiteWinning)
    {
        ...
        int centerDist = Math.Max(3 - losingFile, losingFile - 4) + Math.Max(3 - losingRank, losingRank - 4);
        mopUpScore += centerDist * 10;

        int winningRank = winningKingIdx / 8;
        int winningFile = winningKingIdx % 8;
        int kingDist = Math.Abs(winningRank - losingRank) + Math.Abs(winningFile - losingFile);
    ...
    private bool IsRepetition()
    {
        ...
        int startPly = Math.Max(0, currentPly - 100);
